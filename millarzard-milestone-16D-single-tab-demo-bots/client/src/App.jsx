...
function App() {
  ...
  function addDemoBots(count) {
    if (!demoModeEnabled || !room) return;
    socket.emit("addDemoBots", { roomCode: room.roomCode, count });
  }

  function fillDemoBots(targetPlayerCount) {
    if (!demoModeEnabled || !room) return;
    socket.emit("addDemoBots", { roomCode: room.roomCode, targetPlayerCount });
  }

  function removeDemoBots() {
    if (!demoModeEnabled || !room) return;
    socket.emit("removeDemoBots", { roomCode: room.roomCode });
  }
    ...
    return (
      <main className="app">
        <Lobby
          ...
          demoMode={demoModeEnabled}
          onAddDemoBots={addDemoBots}
          onFillDemoBots={fillDemoBots}
          onRemoveDemoBots={removeDemoBots}
